...
本应用采用兼容普通 Android 应用的发现路径：在用户主动扫描时，读取本机可见的私有 IPv4 接口地址，识别可能的热点网关接口，并在该接口所属的 IPv4 网段内读取可用邻居缓存、执行 mDNS 与 UPnP/SSDP 公开服务发现。主页扫描不进行 TCP 端口连通性检查；端口扫描仅能从某台已发现设备的详情页由用户手动启动。扫描不依赖固定的 `192.168.43.0/24` 网段，因此可兼容厂商使用不同热点地址段的情形。
...
热点模式的设备结果仍来自设备响应的公开网络信号或系统可读的邻居缓存。由于客户端隔离、防火墙、设备休眠、不开启公开服务或厂商限制，已接入热点的设备可能不出现在应用列表；未发现并不等于未连接。应用不进行登录、主动 ARP 请求、MAC 厂商追踪、任意端口范围扫描或漏洞探测。
