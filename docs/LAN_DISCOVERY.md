...
| 8 | 没有可扫描的 Wi‑Fi 或热点网络时查看首页 | 显示可见的本机 IPv4、接口和地址范围；该信息不触发外部设备扫描 |
| 9 | 在无可扫描局域网状态下点击“检测本机 14 个常见端口” | 只检查当前显示的本机 IPv4；不会检查远程地址或端口范围 |
| 10 | 点击首页设置 | 手动同步本地 OUI 注册表；不会在扫描时联网查询 MAC |
...
当没有可用于发现的 Wi‑Fi 或热点局域网时，首页会从当前活动非回环 IPv4 接口中显示本机地址、接口和 CIDR。该展示只用于诊断本机状态，**不会**把蜂窝、链路本地或其他接口变成扫描目标。用户可以主动检查该本机 IPv4 的固定 14 个端口；这是一项本机连通性检查，不会对任何外部设备建立连接。
...
对已发现设备的端口扫描只能从设备详情页启动。固定端口为 `21`、`22`、`23`、`53`、`80`、`443`、`445`、`554`、`631`、`1883`、`3389`、`8080`、`8443` 与 `9100`，每端口只进行短时 TCP 连接，不发送协议载荷或认证请求。无可扫描局域网时的“本机端口检测”只复用这同一清单，并且目标只能是首页已显示的本机 IPv4。在线监测只在应用前台对一台已发现设备运行；没有响应时显示“未确认”，因为服务关闭、防火墙或网络隔离都可能造成无响应。
