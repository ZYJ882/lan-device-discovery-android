...
热点模式下，Android 的默认网络可能仍为蜂窝网络。本应用优先识别本机热点私有 IPv4 接口，并只合并同子网可读邻居缓存、mDNS 与 SSDP 响应。普通应用不能访问系统完整 Soft AP 客户端控制表或 DHCP 租约；热点邻居缓存仅代表当前或近期通信记录，绝不等同于系统设置中显示的完整关联客户端列表。[4]
...
端口扫描只能从已发现设备的详情页启动。固定端口为 `21`、`22`、`23`、`53`、`80`、`443`、`445`、`554`、`631`、`1883`、`3389`、`8080`、`8443` 与 `9100`，每端口只进行短时 TCP 连接，不发送协议载荷或认证请求。在线监测只在应用前台对一台已发现设备运行；没有响应时显示“未确认”，因为服务关闭、防火墙或网络隔离都可能造成无响应。
